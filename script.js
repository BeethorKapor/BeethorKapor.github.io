...
function calculateAge() {
  const birthDate = new Date("2002-10-05"); // 5 ตุลาคม 2545
  const today = new Date();
  const thisYearBirthday = new Date(today.getFullYear(), 9, 5); // เดือน 10 = index 9
  ...
  // if (today >= thisYearBirthday) {
  //   ageText.textContent = "🎉 วันเกิดของคุณ! 🎉";
  // } else {
  // }
...
function updateCountdown() {
  ...
  if (diff > 0) {
    ...
    document.querySelector(".countdown h2").textContent =
      "🎉 สุขสันต์วันเกิด! 🎉";
    document.getElementById("countdown").innerHTML =
      '<p style="font-size: 2em; color: #d4407c;">วันนี้คือวันพิเศษของคุณ! ❤️</p>';
